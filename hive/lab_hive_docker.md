...
```
SELECT driverId, sum(hours_logged), sum(miles_logged) 
FROM timesheet 
GROUP BY driverId;
```
...
Exit the the `hive shell` by typing `exit;`
...
Enter `presto client` by typing the following:

```
presto --server presto-coordinator:8080 --catalog hive --schema default
```

Try running some of the above queries.

![presto](images/013_presto.png)

Exit the the `presto client` by typing `quit`

### Beeline:

Enter `Beeline client` by typing the following:
...
### JDBC clients - DBeaver:

![dbeaver](images/014_dbeaver.png)
